...
def mfcc(file_wav, winlen, ovrlen, pre_coef, nfilter, nftt):
    ...
    #print(numpy.column_stack((t,d,dd)).shape)
...
def enframe(speech, fs, winlen, ovrlen):
     ...
     #print(N,numpy.fix(fs*winlen),numpy.fix(fs*ovrlen))
     if len(speech) < flth:
     ...
     Zerpadlen = (frames-1)*foVr + flth
     #print(frames)

     if len(speech) < Zerpadlen:
        Zerpadsignal = numpy.concatenate((speech, numpy.zeros((Zerpadlen - N))))
     else:
        Zerpadsignal = speech
